...
class Site(object):
    ...
    def make_site(cls,
        ...
        :param searchmask:
            A shell wildcard string to filter in the template file name.
            If present, only templates with the matching filename will be
            processed, e.g. ``*.html``.
        ...
        :param searchmask:
            A shell wildcard string to filter out the static file name.
            If present, only static files with filenames that do not
            match this wildcard will be processed, e.g. ``*.html``.
        ...
        if not os.path.isabs(searchpath) and '://' not in searchpath:
        ...
        if env_kwargs is None:
            env_kwargs = {}
        env_kwargs['loader'] = FSLoader(template_fs=searchpath,
                                                encoding=encoding,
                                                fs_filter=searchmask)
